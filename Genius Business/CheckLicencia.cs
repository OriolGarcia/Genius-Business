...
namespace Genius_Business
{
    class CheckLicencia
    {
       ...
       long idUser;
        ...
        private static readonly string Clave = "GNB-2020-Licenceogs1017";
            ...
            if (CheckEmailKey(Email, Key, macpc))
            {
                ...
                foreach (NetworkInterface adapter in nics)
                {
                    ...
                    if (mac == null && newlicence && offline == false)
                    {
                        try
                        {
                            ...
                            cmd.ExecuteNonQuery();
                             ...
                             sql = "SELECT id_App, email, program_key, mac, limitdate, GETDATE(), id FROM LICENCIAS WHERE email=@email AND mac=@MAC ";
                            ...
                            CrearLiceciaLocal(Email, Key, mac, limitdate, idUser);
                    ...
                    if (adapter.GetPhysicalAddress().ToString() == mac && offline == false)
                    {
                        if (limitdate != new DateTime(0) && limitdate < now)
                        {
                            MessageBox.Show("El producto ha caducado. Por favor compre otra licencia.");
                            return false;
        ...
        private bool CheckEmailKey(String email, String Key, String macpc)
        {
            ...
            try
            {
                ...
                string sql = "SELECT id_App, email, program_key, mac, limitdate, GETDATE(), id FROM LICENCIAS WHERE email=@email AND program_key=@Key AND id_app='" + codeApp + "'";
                SqlCommand cmd = new SqlCommand(sql, con);
                cmd.Parameters.AddWithValue("@email", email);
        ...
        private void CrearLiceciaLocal(String email, String Clave, String MAC, DateTime fechalimite,long  id)
        {
            string lines = email + "\r\n" + "\r\n" + Clave + "\r\n" + "\r\n" + MAC + "\r\n" + "\r\n" + fechalimite.ToString("yyyy-MM-dd HH:mm:ss") + "\r\n" + "\r\n" + id;

            string text = Encriptar(lines);

            System.IO.File.WriteAllText("Security.GNB", text);


            App.idUser = id.ToString();
        }
